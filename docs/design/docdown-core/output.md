...
diagnostics, gaps, and environment facts — into the invariant on-disk layout, and guarantees two
...
- **`IExtractionSink`** — the write surface handed to a backend: `AddImageAsync`, `AddPageAsync`,
  `WriteContentAsync`, `AddContentPartAsync`, and the `Report*` methods. Every `Add*` method returns
...
extraction produced. It has one entry per core artifact — `summary`, `manifest`, `content`, `images`,
`pages` — and each entry (`ArtifactEntry`) records a path, an `ArtifactStatus` of `Present`, `Partial`,
...
(`DD0301` + `DD0702`), images suppressed by `IncludeEmbeddedImages = false` (`DD0201`), whole artifacts
...
- **`IExtractionSink`** (`interface`) — The write surface a backend uses: `AddImageAsync`,
  ...
  `ReportDiagnostic`, `ReportGap`, `ReportEnvironmentFact`, `ReportFound`.
- **`ImageHint`** (`sealed record`) — Extractor-supplied image metadata: a preferred name, media type,
  optional dimensions, source page, source reference, an optional `Transform`, and an optional
  `Description` with its `DescriptionSource` provenance. The naming members are advisory — Core
  allocates the real name — but `Transform` and the description are authoritative provenance, because
  only the extractor knows how it produced the bytes and what, if anything, the document said about the
  image. `ExtractionSink` records the description and its source verbatim, defaulting both to absent,
  and `ManifestWriter` serializes them as the image's `description`/`descriptionSource` metadata.
...
- **`ContentPartKind`** (`enum`) — `Page`, `Sheet`, `Slide`, `Section`, `Attachment`.
...
- **`ArtifactLedger`** (`sealed record`) — The five-entry completeness ledger (`Summary`, `Manifest`,
...
- **`ExtractionManifest`** (`sealed record`, a DTO graph) — The `manifest.json` serialization model
  and its nested records (tool, source, extractor, selection, environment, document, artifacts,
  images, pages, gaps, diagnostics, requested options, failure). Uses only `string`, numeric, `bool`,

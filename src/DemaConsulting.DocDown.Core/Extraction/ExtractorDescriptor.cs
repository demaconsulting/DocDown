...
/// <summary>
///     An immutable description of a registered extractor's identity and declared abilities,
///     captured once at registration time.
/// </summary>
/// <param name="Id">The extractor's stable, unique identifier used for lookup and override.</param>
...
/// <param name="Priority">
///     The extractor's ranking priority; higher values are preferred when other selection
///     criteria tie.
/// </param>
...
///     a non-paginated format with silence rather than a false shortfall.
/// </param>
/// <remarks>
///     A descriptor is a snapshot detached from the live <see cref="IDocumentExtractor"/>
///     instance so selection, reporting, and manifest serialization can reason about an
///     extractor without invoking it or depending on its lifetime. Instances are immutable and
///     thread-safe; the <see cref="SupportedFormats"/> list is captured at construction and not
///     copied defensively, so callers must pass a list they will not mutate.
/// </remarks>
public sealed record ExtractorDescriptor(
    string Id, string DisplayName, IReadOnlyList<DocumentFormat> SupportedFormats,
    ExtractorCapabilities Capabilities, int Priority, bool PageRenderingApplicable = true);

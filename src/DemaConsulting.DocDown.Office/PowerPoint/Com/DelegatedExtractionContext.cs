...
///     <see cref="ExtractionOptions.RenderPages"/> off so the delegated run stays text-only while
///     this backend answers page rendering itself. It also composes the delegate's output through a
///     <see cref="ComposingDelegatedSink"/> so the delegate's "rendering not provided" fact does
///     not contradict the rendering this COM run performs. Immutable after construction and safe to
///     read from the extraction thread.
/// </remarks>
